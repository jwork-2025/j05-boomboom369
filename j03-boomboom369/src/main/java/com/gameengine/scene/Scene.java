...
public class Scene {
     ...
     * 根据名称查找游戏对象列表
     */
    public List<GameObject> findGameObjectsByName(String targetName) {
        if (targetName == null) {
            return Collections.emptyList();
        }
        return gameObjects.stream()
            .filter(obj -> targetName.equals(obj.getName()))
            .collect(Collectors.toList());
    }
     ...
     * 根据名称查找首个匹配的对象
     */
    public GameObject findFirstGameObjectByName(String targetName) {
        if (targetName == null) {
            return null;
        }
        for (GameObject obj : gameObjects) {
            if (targetName.equals(obj.getName())) {
                return obj;
            }
        }
        return null;
